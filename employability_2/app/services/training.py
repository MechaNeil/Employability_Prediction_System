...
from employability_2.app.core.config import LEGACY_MODEL_PATH, MODELS_DIR
from employability_2.app.services.model_registry import activate, get_active, initialize_registry, register_version
...
def _persist_version(model, *, dataset_key: str, mode: str) -> dict[str, object]:
    tmp_path = MODELS_DIR / "_tmp_employability_2.pkl"
...
def train_or_retrain_model(dataset_key: str = "set3", retrain_from_main: bool = False) -> dict[str, object]:
    initialize_registry()
    ...
    return {
        "message": "Employability-2 model training completed.",
...
def activate_model_version(version_name: str) -> dict[str, object]:
    ...
    return {
        "message": "Employability-2 model version activated.",
        "active_version": entry["version_name"],
        "model_path": entry["path"],
    }
...
def retrain_model() -> dict[str, object]:
    initialize_registry()
    return train_or_retrain_model(dataset_key="set3", retrain_from_main=True)


if __name__ == "__main__":
    initialize_registry()
    if not LOCAL_MODEL_PATH.exists():
        print(train_or_retrain_model(dataset_key="set3", retrain_from_main=False))
    else:
        active = get_active()
