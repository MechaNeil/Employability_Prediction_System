...
from employability_1.app.core.config import LEGACY_MODEL_PATH, MODELS_DIR
from employability_1.app.services.model_registry import activate, get_active, initialize_registry, register_version
...
def _persist_version(model, *, dataset_key: str, mode: str) -> dict[str, object]:
    tmp_path = MODELS_DIR / "_tmp_employability_1.pkl"
...
def train_or_retrain_model(dataset_key: str = "set2", retrain_from_main: bool = False) -> dict[str, object]:
    ...
    return {
        "message": "Employability-1 model training completed.",
...
def activate_model_version(version_name: str) -> dict[str, object]:
    ...
    return {
        "message": "Employability-1 model version activated.",
        "active_version": entry["version_name"],
        "model_path": entry["path"],
    }
