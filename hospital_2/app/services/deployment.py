...
from hospital_2.app.core.config import MAIN_SERVER_UPLOAD_URLS, REQUEST_TIMEOUT_SECONDS
...
def deploy_model_to_main_server(version_name: str | None = None) -> dict[str, object]:
    active = get_active()
    if active is None:
        raise FileNotFoundError("No active hospital_2 model to deploy.")
