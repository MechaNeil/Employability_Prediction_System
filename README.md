...
- Docker Compose for employability node simulation
...
2. Push that main model to employability_1 and employability_2 as their local base
3. Retrain locally in each employability
4. Deploy latest employability models back to main_server
...
Build a privacy-preserving, distributed learning workflow where each employability trains locally and only model artifacts are exchanged with the central server.

### High-Level Architecture

- main_server (local): orchestration, aggregation, evaluation, dashboard, model registry
- employability_1 (docker): local train/retrain/evaluate/deploy/delete for employability_1_model
- employability_2 (docker): local train/retrain/evaluate/deploy/delete for employability_2_model
...
- Employabilitys never send raw training data to main_server.
...
### 4.2 Push Base Model to Employabilitys

- Service: main_server.app.services.orchestration -> POST /deploy
- Main server pushes active main_model artifact to:
  - employability_1 /model/upload
  - employability_2 /model/upload
- Employabilitys store uploaded artifact as new local active version

### 4.3 Employability Local Retraining

Each employability retrains locally using /train or /retrain:

- employability_1 default train dataset: Set-2
- employability_2 default train dataset: Set-3
- retrain_from_main=true attempts to initialize from active main_model
- if main model is unavailable, employability falls back to local fresh train

### 4.4 Deploy Employability Models Back to Main

- employability_1: POST /deploy-to-main (model_family employability_1_model)
- employability_2: POST /deploy-to-main (model_family employability_2_model)
...
- Aggregation uses contributors:
  ...
  - active employability_1_model (if available)
  - active employability_2_model (if available)
...
2. Start employability containers
3. Start main_server
4. Train main model
5. Push main model to employabilitys
6. Retrain in employability_1 and employability_2
7. Deploy employability models to main_server
8. Aggregate to update main_model
9. Re-push updated main_model and repeat

### Inter-service Communication Notes

- main_server -> employabilitys for base push uses /model/upload
- employabilitys -> main_server for return deploy uses /remote-models/upload
- Employabilitys include URL fallbacks for main upload endpoint:
...
- GET /status -> Combined runtime status (employabilitys, model artifacts, metrics)
- GET /model-version -> Registry overview
- POST /train -> Train main model
- POST /deploy -> Push active main_model to employabilitys
- POST /retrain-employabilitys -> Informational endpoint (employabilitys retrain locally)
...
- POST /remote-models/upload -> Receive uploaded models from employabilitys/manual forward
...
### Employability Services (ports 8001, 8002)

- GET /health -> Employability status
- GET /status -> Local employability status and version/eval summaries
- GET /model-version -> Active local version + all local versions
- POST /train -> Local train/retrain based on payload
- POST /retrain -> Retrain-focused endpoint (defaults per employability)
...
- 3 model cards (Main, Employability-1, Employability-2)
...
### Employability Dashboard Features
...
# push main model to employabilitys
curl -X POST http://localhost:8000/deploy

# retrain locally at employabilitys
...
# deploy local employability models back to main
...
3. Bi-directional model exchange between main and employabilitys
4. Trainable aggregated main_model version updates
5. Main and employability dashboards for monitoring and control
6. Local-only delete controls on employability model families
